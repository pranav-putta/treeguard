  ...
  simulate = () => {
    ...
    for (var i = 0; i < n; i++) {
      ...
      for (var j = 0; j < this.randInRange(1, 3); j++) {
        let temp = this.skewedRandInRange(10, 200);
        let humidity = this.randInRange(30, 100);
        let airquality = this.randInRange(400, 2000);
        log.push({
          co2: airquality,
          humidity: humidity,
          temp: temp,
              ...
              <Marker
                ...
                onPress={() => {
                  ...
                  if (this.state.selectedNodeIndex != -1) {
                    current[this.state.selectedNodeIndex].color = colors.blue;
                  }
                  current[ind].color = colors.green;

                  this.setState({ nodes: current, selectedNodeIndex: ind });
                }}
              />
            );
          })}
